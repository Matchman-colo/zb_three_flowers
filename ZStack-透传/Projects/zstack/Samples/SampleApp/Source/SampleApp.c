...
#endif
//杩欐鏄姞锟?  
 // Point_To_Point_DstAddr.addrMode = (afAddrMode_t)Addr16Bit;//杩欒浠ｇ爜璁剧疆浜嗙洰鏍囪澶囩殑绔偣鍙枫�傜鐐瑰湪Zigbee涓畾涔変簡璁惧涓婄殑涓嶅悓鍔熻兘鎴栨湇鍔★紝瀹氫箟锟?0
 // Point_To_Point_DstAddr.endPoint = SAMPLEAPP_ENDPOINT;
 // Point_To_Point_DstAddr.addr.shortAddr = 0x0000;//鍙戦�佺粰鍗忚皟锟?//end
...
*********************************************************************/
/*
void SampleApp_SendPointToPointMessage(void)//鑷繁鍐欑殑鍗曟挱鍑芥暟
{
    uint8 device;
    if(SampleApp_NwkState == DEV_ROUTER)
      ...
      device=0x02;//缁堢
    else  device=0x03;//鍑洪敊
    if ( AF_DataRequest( &Point_To_Point_DstAddr, &SampleApp_epDesc,SAMPLEAPP_POINT_TO_POINT_CLUSTERID,
    1, &device, &SampleApp_TransID,AF_DISCV_ROUTE, AF_DEFAULT_RADIUS ) == afStatus_SUCCESS )
    {
