...
void printf_proc()
{
    ...
    flag_data=0;
    printf("1,%d.%d,%d.%d,%d,%d.%d,",wendu/10,wendu%10,shidu/10,shidu%10,BH,(bmp280RawPressure-328200)/1000,(bmp280RawPressure-328200)%1000/10);
    delay_ms(500);
    printf("bb");
}
void uart_jieshou_proc()//串口接受处理函数
{
    if(receive_flag==1)
    {
        ...
        if(strncmp(receive_buf,"aa",2)==0)//如果收到a
        {
            /*
