...
void printf_proc()
{
    ...
    flag_data=0;
    printf("3,%d.%d,%d.%d,%d,%d.%d,",wendu/10,wendu%10,shidu/10,shidu%10,BH,(bmp280RawPressure-386200)/1000,(bmp280RawPressure-386200)%1000/10);
    delay_ms(500);
    printf("aa");
}
void uart_jieshou_proc()//串口接受处理函数
{
    if(receive_flag==1)
    {
        ...
        if(strncmp(receive_buf,"3ybj",4)==0)//发送TH
        {
            flag_bj=1;
            memset(receive_buf,0,5);
            
        }
        else if(strncmp(receive_buf,"3nbj",4)==0)//发送TL
        {
            flag_bj=0;
            memset(receive_buf,0,5);
        }
